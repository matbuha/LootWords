...
- Reward foundation:
  ...
  - coins, stickers, profile avatars, and profile backgrounds are now real reward types with live reward reveal support and persisted balances/ownership
  - duplicate sticker, avatar, and background rewards now convert into coins instead of creating duplicate inventory entries
  - first-owned profile avatars and backgrounds now auto-equip into minimal selected-state fields so later profile UI can build on real equipped data
  - cursor skins and theme packs still remain foundation-only and are not yet active in the live loot pool
- Tap the Word:

...
- `lootwords/index.html` mounts a single root.
- `lootwords/scripts/app.js` is the orchestration layer for routing, rendering, cross-screen actions, and persistence.
- `lootwords/scripts/router.js` keeps navigation hash-based and framework-free.
...
- `lootwords/scripts/storage.js` is the only localStorage entry point.
...
- Screens are plain render functions under `lootwords/scripts/ui/`.
- `app.js` selects a renderer from `SCREEN_RENDERERS`.
- Active screens can optionally return:
  - `destroy()`
  - `advanceTime(ms)`
  - `getDebugState()`
- `lootwords/scripts/core/error-boundary.js` now contains screen destroy/render failures and replaces bad screens with a safe fallback panel instead of blanking the app.

## Game architecture

- `lootwords/scripts/games/game-registry.js` is the single registration point for mini-games.
...
- `lootwords/scripts/core/game-session-manager.js` owns shared per-game stats, recommendation rules, and milestone summaries.

### Add a new mini-game

1. Create a module in `lootwords/scripts/games/`.
...
4. Register the game in `lootwords/scripts/games/game-registry.js`.
5. Verify play -> result -> reward flow in the browser.

## Reward flow

- `lootwords/scripts/core/rewards.js` is the source of truth for reward outcomes.
...
- `lootwords/scripts/core/parent-mode.js` owns category toggles, manual card disable state, reward tuning, active child-mode filtering, and parent summaries.
- Parent UI is split into section renderers under `lootwords/scripts/ui/parent-sections/`.
